...
class ChartingState extends BeatState {
	public var deezNuts = new Map<Int, Int>(); // snap conversion map
	...
	override function create() {
		...
		if (_song.events == null)
			_song.events = [new Song.Event("Init BPM", 0, _song.bpm, "BPM Change")];
	...
	var currentSelectedEventName = "";
	var savedType = "";
	var savedValue = "";
	var currentEventPosition = 0.0;
	...
	function addEventsUI() {
		/*if (_song.events == null)
			_song.events = [new core.Event("Init BPM", 0, _song.bpm, "BPM Change")];

			var firstEvent = "";
	
			if (Lambda.count(_song.events) != 0)
				firstEvent = _song.events[0].name;
			...
			/*var eventSave = new FlxButton(10, 155, "Save Event", function() {
				var pog = new core.Event(currentSelectedEventName, currentEventPosition, HelperFunctions.truncateFloat(Std.parseFloat(savedValue), 3), savedType);
				trace("trying to save " + currentSelectedEventName);

				var obj = containsName(pog.name,_song.events);
				...
				if (obj != null)
					_song.events.remove(obj);
				_song.events.push(pog);

				trace(_song.events.length);
				TimingStruct.clear();
			...
			/*var eventAdd = new FlxButton(95 ,155, "Add Event", function() {
				var pog = new core.Event("New Event " + HelperFunctions.truncateFloat(curDecimalBeat, 3),HelperFunctions.truncateFloat(curDecimalBeat, 3), _song.bpm, "BPM Change");
			...
			/*var eventRemove = new FlxButton(180,155,"Remove Event", function() {
				...
				var obj = containsName(listOfEvents.selectedLabel, _song.events);
				...
				var firstEvent = _song.events[0];
				if (firstEvent == null) {
					_song.events.push(new core.Event("Init BPM", 0, _song.bpm, "BPM Change"));
					firstEvent = _song.events[0];
				}
			...
			/*var updatePos = new FlxButton(150, 120, "Update Pos", function() {
				var obj = containsName(currentSelectedEventName, _song.events);
				if (obj == null)
					return;
			...
			for (event in _song.events) {
				...
				trace(value);
				var eventt = new core.Event(name, pos, value, type);
				chartEvents.push(eventt);
				listofnames.push(name);
			}

			_song.events = chartEvents;
			if (listofnames.length == 0)
				listofnames.push("");

			if (_song.events.length != 0)
				firstEventObject = _song.events[0];
			trace("bruh");*/
			...
			/*listOfEvents = new FlxUIDropDownMenu(10, 20, FlxUIDropDownMenu.makeStrIdLabelArray(listofnames, true), function(name:String) {
				var event = containsName(listOfEvents.selectedLabel, _song.events);
				if (event == null)
					return;
			...
			eventName.callback = function(string:String, string2:String) {
				...
				obj = containsName(string, _song.events);
				if (obj != null)
					return;
				obj = containsName(currentSelectedEventName, _song.events);
				obj.name = string;
				currentSelectedEventName = string;
			};*/
	...
	function addSectionUI():Void {
		...
		/*var clearSectionButton = new FlxButton(10, 150, "Clear Section", clearSection);
		...
		var startSection = new FlxButton(10, 85, "Play Here", function() {
			Core.song = _song;
	...
	function addNoteUI():Void {
		...
		var applyLength = new FlxButton(10, 100, 'Apply Data');
	}
		...
		if (id == FlxUICheckBox.CLICK_EVENT) {
			...
			switch (label) {
				case "P1 Alternate Animation":
					getSectionByTime(Conductor.songPosition).isP1Alt = check.checked;
				case "P2 Alternate Animation":
					getSectionByTime(Conductor.songPosition).isP2Alt = check.checked;
			}
	...
	var doSnapShit = false;

	public var currentBPM = 0.0;
	...
	public var waitingForRelease = false;
	...
	public var selectInitialX = 0.0;
	public var selectInitialY = 0.0;
	...
	function updateHeads():Void {
		/*if (check_mustHitSection.checked) {
			leftIcon.animation.play(_song.players[1]);
			rightIcon.animation.play(_song.players[2]);
		} else {
			leftIcon.animation.play(_song.players[2]);
			rightIcon.animation.play(_song.players[1]);
		}*/
	}
	...
	function shiftNotes(measure = 0, step = 0, ms = 0):Void {
		...
		for (daSection1 in 0..._song.notes.length)
			newSong.push(newSection(16, _song.notes[daSection1].mustHitSection, _song.notes[daSection1].isP1Alt, _song.notes[daSection1].isP2Alt));
		for (daSection in 0...(_song.notes.length)) {
			...
			newSong[aimtosetsection].isP1Alt = _song.notes[daSection].isP1Alt;
			newSong[aimtosetsection].isP2Alt = _song.notes[daSection].isP2Alt;
	...
	function loadJson(song:String):Void {
		...
		var format = StringTools.replace(Core.song.song.toLowerCase(), " ", "-");
		switch (format) {
			case 'Dad-Battle': format = 'Dadbattle';
			case 'Philly-Nice': format = 'Philly';
		}
		//Core.song = Song.loadFromJson(format + difficultyArray[Core.storyDifficulty], format);
